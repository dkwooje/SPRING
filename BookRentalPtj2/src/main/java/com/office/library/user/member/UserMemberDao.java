...
public class UserMemberDao {
	...
	public int insertUserAccount(UserMemberVo userMemberVo) {
		...
		String sql = "INSERT INTO tbl_user_member(u_m_id, "
											   ...
											   + "u_m_phone, "
											   + "u_m_reg_date, "
											   + "u_m_mod_date) VALUES(?, ?, ?, ?, ?, ?, NOW(), NOW())"; //등록날짜와 수정날짜는 생성당시 동일
		...
		try {
			...
			result = jdbcTemplate.update(sql, 
											 ...
											 passwordEncoder.encode(userMemberVo.getU_m_pw()), //인코딩: 사람문자를 컴퓨터문자(0,1)로 바꾸는 것
	...
	public UserMemberVo selectUser(int u_m_no) {
		...
		try {userMemberVos = jdbcTemplate.query(sql, new RowMapper<UserMemberVo>() {
